...
The dataset for this assignment can be found as a csv file under `us_state_vaccinations` in the `data` folder of the repository.

```{r load_dataset}
cov_19_vaccine <- read.csv("data/us_state_vaccinations.csv")
```

This is state-by-state (and other U.S. territories) data on United States COVID-19 vaccinations from the United States Centers for Disease Control and Prevention (CDC). DUring writing this proposal, data were only available up until March 8th 2021, hence, we may update our data set once again when working more closely for the presentation to ensure for an accurate analysis.However, we will refrain continuously updating our data as it could disrupt our work. 
